...
import { ArrowLeft, Save, Loader2, Calendar, FileEdit } from "lucide-react";
...
export default function EditRequestPage() {
    ...
    fetch(`/api/requests/${requestId}`)
      ...
      .then((data) => {
        setFormData({
          ...
          careType: data.care_type || "companion",
          personAge: data.person_age?.toString() || "",
          requirements: data.requirements || [],
          urgency: data.urgency || "medium",
          hourlyRate: data.hourly_rate?.toString() || "",
          totalHours: data.total_hours?.toString() || "",
          isRecurring: data.is_recurring || false,
          weekdays: data.weekdays || [],
          requestDate: new Date(data.request_date).toISOString().split("T")[0],
      ...
      .catch((error) => {
        ...
        errorAlert.loading("la información de la solicitud");
        setIsLoading(false);
      });
  }, [requestId]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const dataToValidate: UpdateRequestInput = {
      ...
      // Validar con Zod
      const validatedData = updateRequestSchema.parse(dataToValidate);
      ...
      await successAlert.updated("La solicitud");
      ...
      if (error && typeof error === "object" && "issues" in error) {
        ...
        errorAlert.saving("No se pudo actualizar la solicitud");
      }
    } finally {
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <Card className="p-6 md:p-8">
          <form onSubmit={handleSubmit} className="space-y-8">
            {/* Step 0: Estado de la Solicitud */}
            <FormSection
              title="Estado de la Solicitud"
              description="Gestiona el estado actual de tu solicitud"
              step={0}
              totalSteps={4}
              isCompleted
            >
